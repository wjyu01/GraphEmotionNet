...
def augmentation_MDD(feature_seqence,label_seqence,alpha=0.5):
    augment_data=[]
    augment_label=[]
    ...
    for j in range(len(feature_seqence)):
        index=np.random.randint(0,len(feature_seqence),2)
        weight_sequence=0.5*np.ones(2).reshape((1,2))
        lam=np.random.beta(alpha,alpha)
        weight_sequence[0,0]=lam
        weight_sequence[0,1]=1-lam
        augment_data.append(np.dot(weight_sequence,feature_seqence[index,:]))
        augment_label.append(label_seqence[j,:])
...
def get_dataset_MDD_aug(test_id, session, sorce_labeled, sorce_unlabeled, target, parameter):
    alpha = parameter['alpha']
    if session==1:
        path = 'E:\\zhourushuang\\MDD\\ECdata'
    else:
        path = 'E:\\zhourushuang\\MDD\\EOdata'
    os.chdir(path)
    ...
    for info in os.listdir(path):
        ...
        sub_index_str, _ = os.path.splitext(info)
        ...
        if sub_index in sorce_labeled:
            ...
            feature_labeled_aug, label_labeled_aug = augmentation_MDD(feature_labeled_origin, label_labeled_origin, alpha)
        ...
        if sub_index in sorce_unlabeled:
            ...
            feature_unlabeled_aug, label_unlabeled_aug = augmentation_MDD(feature_unlabeled_origin, label_unlabeled_origin, alpha)
        ...
        if sub_index in target:
            ...
            feature_aug, label_aug = augmentation_MDD(feature_origin, label_origin, alpha)
    ...
    target_feature, target_label = np.vstack(feature_list_target), np.vstack(label_list_target)
    ...
    target_feature_aug, target_label_aug = np.vstack(feature_list_target_aug), np.vstack(label_list_target_aug)
...
def get_dataset_aug(test_id,session,video,parameter):
    alpha=parameter['alpha']
    path='E:\\zhourushuang\\de_feature\\feature_for_net_session'+str(session)+'_LDS_de'
    os.chdir(path)
    feature_list_source_labeled=[]
    label_list_source_labeled=[]
    feature_list_source_unlabeled=[]
...
def train_model(target_len, loader_train_labeled,loader_train_unlabeled, loader_test,model,dann_loss, optimizer,hidden_4,epoch,batch_size,parameter,threshold_update=True):
    ...
    for i in range(T):
        ...
        with torch.no_grad():
            ...
            X,Y=torch.cat((x_s,x_un)),torch.cat((labels_s,un_predict.to(labels_s)))
        if epoch>=100:
            source_predict,feature_source_f,feature_target_f,sim_matrix,sim_matrix_target = model(X,x_t,Y.to(X), proto_wo_U=0)
        else:
            source_predict, feature_source_f, feature_target_f, sim_matrix, sim_matrix_target = model(X, x_t, Y.to(X), proto_wo_U=1)
        eta=0.00001
...
def train_and_test_GAN(test_id, sorce_labeled, sorce_unlabeled, target,
                       max_iter,parameter,session,threshold_update=True):
    setup_seed(20)
    ...
    target_set, source_set_labeled, source_set_unlabeled = get_dataset_MDD_aug(test_id, session, sorce_labeled, sorce_unlabeled, target, parameter)
    ...
    for epoch in range(max_iter):
        ...
        model.train()
        cls_loss_sum,transfer_loss_sum=train_model(len(target_set['feature']), loader_train_labeled,loader_train_unlabeled,loader_test,model,dann_loss,optimizer,hidden_4,epoch,BATCH_SIZE_Target,parameter,threshold_update)
        if epoch >= 100:
            source_acc,source_nmi=model.cluster_label_update(source_features.cuda(),source_labels.cuda(), proto_wo_U=0)
        else:
            source_acc, source_nmi = model.cluster_label_update(source_features.cuda(), source_labels.cuda(), proto_wo_U=1)
        model.eval()
        ...
        if target_acc > best_acc:
            os.chdir('E:\\zhourushuang\\MDD\\result')
            ## 以下代码用于保存模型参数，以便绘制混淆矩阵，注意：如果要完整保存模型必须使用代码中的checkpoint参数
            #            checkpoint(model,'bestmodel_simpair_batch96_upper90_lower50_'+str(test_id)+'_session'+str(session)+'.pkl','save')
            checkpoint(model, 'modelfull_semi_{}_MDD_batch48_full_Sratio{}.npy'.format(task, parameter['S_ratio']) + str(test_id) + '.pkl', 'save')
            torch.save(model.state_dict(), 'model_semi_{}_MDD_batch48_full_Sratio{}.npy'.format(task, parameter['S_ratio']) + str(test_id) + '.pkl')
...
def main(update_threshold,parameter,session):
    setup_seed(20)
    nflod = 10
    skf = KFold(n_splits=nflod, shuffle=True, random_state=20)

    H = np.array(list(range(1, 28)))
    H_train, H_test = [],[]
    H_train_labeled, H_train_unlabeled = [], []
    for fold, (a, b) in enumerate(skf.split(range(len(H)))):
        H_train.append(H[a])
        H_test.append(H[b])
        r = random.sample(H[a].tolist(), int(len(a) * parameter['S_ratio']))
        r_u = [x for x in H[a] if x not in r]
        H_train_labeled.append(r)
        H_train_unlabeled.append(r_u)

    MDD = np.array(list(range(28, 57)))
    MDD_train, MDD_test = [],[]
    MDD_train_labeled, MDD_train_unlabeled = [], []
    for fold, (a, b) in enumerate(skf.split(range(len(MDD)))):
        MDD_train.append(MDD[a])
        MDD_test.append(MDD[b])
        r = random.sample(MDD[a].tolist(), int(len(a) * parameter['S_ratio']))
        r_u = [x for x in MDD[a] if x not in r]
        MDD_train_labeled.append(r)
        MDD_train_unlabeled.append(r_u)


    max_iter=1000
    best_acc_mat=np.zeros(nflod)
    transfer_loss_curve=np.zeros((nflod,max_iter))
    cls_loss_curve=np.zeros((nflod,max_iter))
    source_acc_curve=np.zeros((nflod,max_iter))
    target_acc_curve=np.zeros((nflod,max_iter))
    source_nmi_curve=np.zeros((nflod,max_iter))
    target_nmi_curve=np.zeros((nflod,max_iter))
    for i in range(nflod):

        sorce_labeled = np.hstack((H_train_labeled[i], MDD_train_labeled[i]))
        sorce_unlabeled = np.hstack((H_train_unlabeled[i], MDD_train_unlabeled[i]))
        target = np.hstack((H_test[i], MDD_test[i]))

        best_acc,cls_loss_list,source_acc_list,source_nmi_list,\
        target_acc_list,target_nmi_list,transfer_loss_list=train_and_test_GAN(i,sorce_labeled, sorce_unlabeled, target,
                                                                              max_iter,parameter,session,update_threshold)
...
parameter={'hidden_1':64,'hidden_2':64,'num_of_class':2,'cluster_weight':2,'low_rank':32,'upper_threshold':0.9,'lower_threshold':0.5,
           'boost_type':'linear','video':3,'temp':0.9,'batch_size':48,'alpha':0.5, 'S_ratio':0.66, 'session':1}
best_acc_mat,cls_loss_curve,transfer_loss_curve,source_acc_curve,source_nmi_curve,target_acc_curve,target_nmi_curve=main(True,parameter,parameter['session'])
if parameter['session']==1:
    task = 'EC'
else:
    task = 'EO'
...
os.chdir('E:\\zhourushuang\\MDD\\result')
np.save('result_list_semi_{}_MDD_batch48_full_Sratio{}.npy'.format(task, parameter['S_ratio']),result_list)

c=np.load('result_list_semi_{}_MDD_batch48_full_Sratio{}.npy'.format(task, parameter['S_ratio']),allow_pickle=True).item()
