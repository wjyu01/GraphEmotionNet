...
def train_STGCN_contrast(subject_id, parameter, net_params, source_loaders, target_loader):
    setup_seed(20)
    device = net_params['DEVICE']
    model = models.STGCN_contrastNet(net_params).to(device)
...
def test_STGCN(subject_id, epoch, model, target_loader, parameter):
    model.eval()
...
def cross_subject(target_set, source_set_labeled, session_id, subject_id, parameter, net_params):
    ...
    acc = train_STGCN_contrast(subject_id,parameter,net_params,source_loaders=source_loaders,target_loader=target_loader)
    return acc
